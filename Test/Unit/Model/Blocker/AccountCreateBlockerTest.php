...
class AccountCreateBlockerTest extends TestCase
{
    ...
    public function setup()
    {
        ...
        $this->model = $this->objectManager->getObject(
            \PrOOxxy\SpamFilter\Model\Blocker\AccountCreateBlocker::class,
            [
    ...
    public function testExecuteWithLinkInFirstname()
    {
        ...
        $dataPosts = ['firstname' => 'string with link'];

        $this->validator->method('stringHasLink')->willReturn(true);

        $this->expectException(LocalizedException::class);
        $this->expectExceptionMessage('firstname contains web link');
    ...
    {
        $dataPosts = ['lastname' => 'ﺏﺕﺙﺝ'];

        $this->validator->method('isStringMatchingBlockedAlphabet')->with($dataPosts['lastname'])->willReturn(true);

        $this->validator->method('getAlphabetByString')->with($dataPosts['lastname'])->willReturn('Arabic');

        $this->expectException(LocalizedException::class);
        $this->expectExceptionMessage('lastname contains Arabic characters which are forbidden');
        $this->model->execute($dataPosts);
